<template>
					...
					<div class="box-inner js-checkout-address-panel ">
						<div class="address-common-table js-multiple-address-panel">
							<ul class="address-item-list clear js-address-item-list">
								<li class="js-choose-address" :class="{'selected-address-item':takeindex==index}" v-for="(item,index) in take" @click="choosereceive(index)">
								...
								<li class="add-address-item js-add-address" @click="opentake">
									<p>使用新地址</p>
								</li>
							</ul>
						</div>
				...
				<div class="box-inner ui-goods-cart">
					...
					<div class="cart-table">
						<div class="cart-group js-cart-group">
							<div class="cart-items" v-for="(item,index) in goodscheck">
    ...
    <take v-if="showtake" @close="closetake"></take>
	</div>
</template>
  ...
  import take from '../components/take'
    ...
    components:{
      take
		...
		computed:{
			goodscheck(){
				return this.$store.getters.goodscheck
			},
			checkprice(){
      ...
      closetake(){
        this.showtake=false,
        this.$store.state.take.forEach((receive,index)=>{
                if(receive.default){
                    this.takeindex=index
                }
        })
      ...
      submitorder(){
        ...
        let receiveinfo=this.take[this.takeindex]
        ...
        let data={
          orderid:invoicedate.getTime(),
          goodsdata:this.goodscheck,
          price:this.checkprice,
          freight:this.freight,
          invoicename:this.invoice.name,
          invoicedate:invoicedate.getFullYear()+'-'+month+'-'+day,
          take:receiveinfo,
          ispay:false
        }
        this.$store.commit('submitorder',data)
        this.$router.push({name:'order',query:{orderid:data.orderid}})
      }
		},
		created(){
			this.$store.state.take.forEach((receive,index)=>{
