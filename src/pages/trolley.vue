<template>
	<div id="main" class="hander-car">
		<div class="store-content">
					...
					<div v-else>
						...
						<div class="cart-table">
							<div class="cart-group">
								<!--购物列表-->
								<div class="cart-top-items" v-for="(item,index) in cartdata">
									<div class="cart-items">
										<div class="items-choose">
											<span class="blue-checkbox-new" :class="{'checkbox-on':item.check}" @click="checkgoods(item.sku_id)"><a></a></span>
										</div>
										...
										<div class="item-cols-num">
											<div class="select js-select-quantity">
												<span class="down" :class="{'down-disabled':item.count<1}" @click="redcount(item.sku_id)">-</span>
												<span class="num">
													{{item.count}}
												</span>
					...
					<div class="cart-bar-operation">
						<div>
							<div class="choose-all js-choose-all">
								<span class="blue-checkbox-new" :class="{'checkbox-on':allcheck}"  @click="checkall(allcheck)"><a></a></span>
								全选
							</div>
							<div class="delete-choose-goods" @click="delcheck">删除选中的商品</div>
						</div>
					</div>
					<div class="shipping">
            ...
            <router-link class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn':checkcount<=0}" tag="span" to="/balance">
              <a>现在结算</a>
            </router-link>
					</div>
				</div>
...
<script>
	export default {
		...
		methods:{
			...
			redcount(id){
				this.$store.commit('redcount',id)
			},
			checkgoods(id){
				this.$store.commit('checkgoods',id)
			},
			checkall(allcheck){
				this.$store.commit('checkall',allcheck)
			},
			delcheck(){
				this.$store.commit('delcheck')
			}
		}
	}
</script>
