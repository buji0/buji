			...
			<div class="item-box">
				<div class="gallery-wrapper">
					<div class="gallery">
						<div class="thumbnail">
							<ul>
								<li :class="{'on':index===imageindex}" v-for="(image,index) in goodsinfo.ali_images" @click="tabimg(index)"><img :src="image+'?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
							</ul>
						</div>
						<div class="thumb">
							<ul>
								<li :class="{'on':index===imageindex}" v-for="(image,index) in goodsinfo.ali_images"><img :src="image+'?x-oss-process=image/resize,w_440/quality,Q_90/format,webp'"></li>
							</ul>
						</div>
					</div>
				</div>
				<div class="banner">
					<div class="sku-custom-title">
						<div class="params-price">
							<span><em>¥</em><i>{{goodsinfo.price}}</i></span>
						</div>
						<div class="params-info">
							<h4>{{goodsinfo.title}}</h4>
							<h6>{{goodsinfo.sub_title}}</h6>
						</div>
					</div>
					<div class="sku-dynamic-params-panel">
						<div class="sku-dynamic-params clear">
							...
							<ul class="params-colors">
								<li :title="color.color" class="cur" v-for="(color,index) in goodsinfo.sku_list">
									<router-link :to="{name:'detail',query:{goodsid:color.id}}">
						...
						<div class="sku-dynamic-params clear">
							...
							<div class="params-detail clear">
								<div class="item-num js-select-quantity">
									<span class="down" :class="{'down-disabled':count<1}" @click="redcount">-</span>
									<span class="num">{{count}}</span>
									<span class="up" :class="{'up-disabled':count>goodsinfo.limit_num}" @click="addcount">+</span>
								</div>
							</div>
						</div>
					</div>
...
<script>
	...
	export default {
		...
		methods:{
			...
			addcart(){
				let goodsdata={info:this.goodsinfo,count:this.count}
				this.$store.commit('add',goodsdata)
			},
			addcount(){
                this.count++
                if(this.count>this.goodsinfo.limit_num){
                    this.count=this.goodsinfo.limit_num
                }
			},
			redcount(){
                this.count--
		...
		computed:{
			goodsinfo(){
				let goodsinfo=this.detaildata.filter(item=>{
				    return Number(item.sku_id)===Number(this.goodsid)
			    })[0]
			    return goodsinfo
			}
		},
