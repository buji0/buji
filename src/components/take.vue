          ...
          <div class="module-dialog clear module-dialog-address module-dialog-show">
            <div class="dialog-panel">
                <div class="topbar">
                    ...
                    <span class="dialog-close" @click="closetake">x</span>
                </div>
                                ...
                                <div class="address-form">
                                    <div class="module-form-row">
                                        <div class="form-item-v3">
                                            <input type="text" class="js-verify" placeholder="收货人姓名" v-model="take.name">
                                            <div class="verify-error"></div>
                                        </div>
                                    </div>
                                    <div class="module-form-row">
                                        <div class="form-item-v3" :class="{'form-invalid-item':phoneerror}">
                                            <input type="text" class="js-verify" placeholder="手机号" v-model="take.phone" @blur="losephone" @focus="getphone">
                                    ...
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 area-code-w fn-left form-valid-item">
                                            <input type="text" class="js-verify js-address-area-code" placeholder="区号（可选）"  v-model="take.areaCode">
                                            <div class="verify-error"></div>
                                        </div>
                                        <div class="form-item-v3 telephone-w fn-right form-valid-item">
                                            <input type="text" class="js-verify js-address-telephone" placeholder="固定电话（可选）" v-model="take.landLine">
                                            <div class="verify-error"></div>
                                        </div>
                                    </div>
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item province-wrapper">
                                            <select name="province_code" class="province select-province js-form-province js-verify" v-model="take.provinceId">
                                                <option value="0">请选择省份</option>
                                                <option :value="item.area_id" v-for="(item,index) in takedata">{{item.area_name}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                            <select class="city select-city js-form-city js-verify" v-model="take.cityId">
                                                <option value="0">请选择城市</option>
                                                <option :value="item.area_id" v-for="(item,index) in citydata">{{item.area_name}}</option>
                                            </select>
                                        </div>
                                        <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                                            <select class="city select-city js-form-city js-verify" v-model="take.countyId">
                                                <option value="0">请选择区县</option>
                                                <option :value="item.area_id" v-for="(item,index) in countydata">{{item.area_name}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="module-form-row">
                                        <div class="form-item-v3">
                                            <input type="text" class="js-verify" placeholder="详细地址，如街道名称，楼层，门牌号码等" v-model="take.add">
                                            <div class="verify-error"></div>
                                        </div>
                                    </div>
                                    <div class="module-form-row fn-clear">
                                        <input type="checkbox" class="hide">
                                        <span class="blue-checkbox" :class="{'blue-checkbox-on':take.default}" @click="checkdefault"></span>设为默认
                                    </div>
                                    <div class="dialog-blue-btn big-main-btn js-verify-address" :class="{'disabled-btn':!isall}" @click="submitinfo">
                                        <a>保存</a>
                                    </div>
                                </div>
                            </div>
...
<script>
    import takedata from '../data/take'
    export default {
        data(){
            return {
                isall:false,
        ...
        methods:{
            ...
            checkdefault(){
                this.take.default=!this.take.default
            },
            inspecttake(){
                if(this.take.name&&this.take.phone&&this.take.province&&this.take.city&&this.take.county&&this.take.add){
                    this.isall=true
                }else{
                    this.isall=false
                }
            ...
            submitinfo(){
                if(this.isall){
                    this.$store.commit('submitinfo',this.take)
                    this.$emit('close')
                }
            }
        },
        computed:{
            citydata(){
                let citydata=[]
                this.takedata.forEach((province)=>{
                    if(province.area_id===this.take.provinceId){
                        this.take.province=province.area_name
                        citydata=province.city_list
                    }
                })
                if(this.take.provinceId==='0'){
                    this.take.cityId=0
                    this.take.countyId=0
                }
                return citydata
            },
            countydata(){
                ...
                this.citydata.forEach((city)=>{
                    if(city.area_id===this.take.cityId){
                        this.take.city=city.area_name
                        countydata=city.county_list
                    }
                })
                if(this.take.cityId==='0'){
                    this.take.countyId=0
                }
                return countydata
            }
        ...
        watch:{
            'take.countyId'(){
                this.countydata.forEach((county)=>{
                    if(county.area_id===this.take.countyId){
                        this.take.county=county.area_name
                    }
                })
            },
            take:{
